...
def _check_if_holder(contract_adress, 
    ...
    for day in list_sorted_days:
        ...
        while not found_holder:
            ...
            if _check_if_holder(max_key):
                ...
                dict_percentage_holders[day] = [max_key, dict_current_day.get(max_key)]
    return dict_percentage_holders

def _filter_df_for_training_days(df, date_col, ico_start_date, ico_end_date):
        if ico_start_date:
            return df.loc[
                (df[date_col] >= ico_start_date)
                & (df[date_col] < ico_end_date)
            ]
        else:
...
class ICOParser:
    def __init__(self, 
        ...
        df_for_resample[date_column] = pd.to_datetime(df_for_resample[date_column])
        ...
        self.df_resample_day = df_for_resample.resample('D', on=date_column).sum()
        self.ico_start_date = datetime.strptime(
            ico_start_date, '%Y-%m-%d'
    ...
    def get_newbiers_dataframe(self):
        ...
        self.df_newbiers = self.df[self.df.FROM_ADDRESS_BLOCKCHAIN.isin(list_newbiers)]
        self.df_newbiers_resample = self.df_newbiers.resample('D', on=self.date_column).sum()


    def get_array_daily_transactions(self):
        df_resample_func = self.df_resample_day.reset_index()
        df_resample_func['BLOCK_TIMESTAMP'] = df_resample_func['BLOCK_TIMESTAMP'].dt.date
    ...
    def get_balance(self):
        ...
        dataframe = _set_dataframe_max_date(self.df, self.date_column, self.ico_end_date) 
        ...
        for delta in range(days_activity):
            ...
            dict_user_balance = {}
            for user in set(list(df_current_date.FROM_ADDRESS.unique()) + list(df_current_date.TO_ADDRESS.unique())):
                to_adress_value = df_current_date.loc[df_current_date.TO_ADDRESS == user].VALUE.sum()
                from_adress_value = df_current_date.loc[df_current_date.FROM_ADDRESS == user].VALUE.sum()
                dict_user_balance[user] = to_adress_value - from_adress_value
            dict_user_balance_sorted = {k: v for k, v in sorted(dict_user_balance.items(), key=lambda item: item[1])}
    ...
    def get_cumsum_daily_percentage(self):
        ...
        dict_percentage_holders = {}
        for day in list_sorted_days:
            total_value = sum([val for val in list(dict_cumsum_balance.get(day).values()) if val > 0])
            ...
            for user in dict_current_day.keys():
                if dict_current_day.get(user) > 0:
                    dict_daily_percentage[user] = dict_current_day.get(user)/total_value
    ...
    def get_biggest_holder_dict(self):
        self.dict_perc_biggest_holder = _get_biggest_holder(self.dict_percentage_holders)
    
    def get_biggest_holder_array(self):
        self.array_biggest_holder = [value[1] for key, value in self.dict_perc_biggest_holder.items()]
    ...
    def get_gas_ratio_array(self):
        if self.df_newbiers_resample:
            self.df_newbiers_resample['GAS_RATIO'] = self.df_newbiers_resample['RECEIPT_GAS_USED'] / self.df_newbiers_resample['GAS']
            self.gas_ratio_array = self.df_newbiers_resample.GAS_RATIO.to_list()
        else:
            print('self.df_newbiers_resample does not exist.\nPlease run self.get_newbiers_dataframe().')
    ...
    def get_daily_number_of_new_holder(self, max_date=None):
        ...
        for day in list_sorted_days:
            ...
            dict_result[day] = {'total_users': total_users,
    ...
    def get_array_perc_new_holders(self):
        ...
        value.get('percentage') for key, value in self.dict_daily_new_holders.items()]
